...
async def export_pdf(meeting_id: str, db: AsyncSession = Depends(get_db)):
    data = await _gather_data(meeting_id, db)
...
async def export_json(meeting_id: str, db: AsyncSession = Depends(get_db)):
    """Export full meeting data as JSON."""
    data = await _gather_data(meeting_id, db)
    return data


async def _gather_data(meeting_id: str, db: AsyncSession) -> dict:
