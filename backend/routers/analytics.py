...
from db.models import User, Meeting, Segment, Speaker, MeetingSummary
...
async def get_analytics(meeting_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def get_summary(meeting_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    meeting = await db.get(Meeting, meeting_id)
    if not meeting or meeting.user_id != current_user.id:
        raise HTTPException(404, "Meeting not found")
...
async def get_segments(meeting_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    meeting = await db.get(Meeting, meeting_id)
    if not meeting or meeting.user_id != current_user.id:
        raise HTTPException(404, "Meeting not found")
...
async def get_global_analytics(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    # Fetch all meetings for the user
    result = await db.execute(
        select(Meeting).where(Meeting.user_id == current_user.id)
    )
    ...
    # Simple heuristic-based aggregate stats
    pos_count = len([m for m in meetings if m.word_count > 500])
    neg_count = len([m for m in meetings if m.word_count < 100])
    ...
    return {
        ...
        "sentiment_distribution": {
            "positive": round((pos_count / len(meetings)) * 100, 1) if meetings else 50,
            "negative": round((neg_count / len(meetings)) * 100, 1) if meetings else 10,
            "neutral": 40
        },
        "recent_activity": [
            {"id": m.id, "title": m.title, "date": m.started_at, "status": m.status}
            for m in meetings[:5]
        ]
    }
