...
def transcribe_file_generator(audio_path: str, language: Optional[str] = None):
    """Transcribe yielding segments one by one."""
    model = get_model()

    if model == "mock":
        yield _mock_transcribe(audio_path)["segments"][0]
        return
    ...
    for seg in segments_gen:
        ...
        yield {
            ...
            "confidence": max(0.0, 1.0 + getattr(seg, "avg_logprob", 0.0)),
        }
